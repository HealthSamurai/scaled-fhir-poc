...
(defn db-spec-from-env
  ([sufix]
   {:host (or (env :pghost sufix) (env :pghost))
    :port (or (env :pgport sufix) (env :pgport))
    :user (or (env :pguser sufix) (env :pguser))
    :database (or (env :pgdatabase sufix) (env :pgdatabase))
    :password (or (env :pgpassword sufix) (env :pgpassword))
    :sufix sufix})
  ([]
   {:host (env :pghost)
    :port (env :pgport)
